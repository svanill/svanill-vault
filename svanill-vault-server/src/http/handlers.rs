...
pub fn render_500<B>(mut res: ServiceResponse<B>) -> actix_web::Result<ErrorHandlerResponse<B>> {
    ...
    if let Some(orig_error) = resp.error() {
        if orig_error.as_error::<VaultError>().is_some() {
            // Send error to Sentry using capture_message instead of
            // capture_error because I didn't manage to get a
            // stacktrace otherwise.
    ...
    *resp.status_mut() = v_error.status_code();
    ...
    let resp = resp.set_body(v_error.to_string());

    let res = ServiceResponse::new(req, resp)
        .map_into_boxed_body()
        .map_into_right_body();
...
pub fn render_40x<B>(mut res: ServiceResponse<B>) -> actix_web::Result<ErrorHandlerResponse<B>> {
